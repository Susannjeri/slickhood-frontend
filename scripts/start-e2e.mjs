import { cpSync, existsSync, mkdirSync } from "node:fs";
import { spawn } from "node:child_process";

const standalone = ".next/standalone/server.js";
// Source-level regression tests must not silently reuse a stale production build.
// A caller may opt into a freshly-built standalone artifact explicitly.
const useProductionBuild = process.env.E2E_USE_STANDALONE === "true";
const useStandalone = useProductionBuild && existsSync(standalone);
if (useStandalone) {
  mkdirSync(".next/standalone/.next", { recursive: true });
  cpSync(".next/static", ".next/standalone/.next/static", { recursive: true, force: true });
  if (existsSync("public")) cpSync("public", ".next/standalone/public", { recursive: true, force: true });
}
const command = useStandalone ? process.execPath : (process.platform === "win32" ? "npm.cmd" : "npm");
const args = useStandalone
  ? [standalone]
  : ["run", useProductionBuild ? "start" : "dev", "--", "--hostname", "127.0.0.1", "--port", "3100"];
const child = spawn(command, args, {
  stdio: "inherit",
  shell: process.platform === "win32" && !useStandalone,
  env: { ...process.env, HOSTNAME: "127.0.0.1", PORT: "3100" },
});

for (const signal of ["SIGINT", "SIGTERM"]) {
  process.on(signal, () => child.kill(signal));
}

child.on("exit", code => process.exit(code ?? 1));
