...
4. The authenticated canonical Git remotes are now restored and both release checkouts contain `origin/main`. The backend release branch is published at `origin/release/helpdesk-listings-tax-integrated-20260902`; publish the frontend release branch through the protected workflow before promotion (it is currently local only).
