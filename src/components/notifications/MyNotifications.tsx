      ...
      <div className="space-y-3">{items.map(item => {
        ...
        return <article key={item.id} className={`rounded-xl border p-5 shadow-sm ${item.read ? "bg-white" : "border-orange-200 bg-orange-50/40"}`}>
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            ...
            {actionUrl && <Button asChild size="sm"><Link href={actionUrl} target={actionUrl.startsWith("https://slickhood.com/property/") ? "_blank" : undefined} rel={actionUrl.startsWith("https://slickhood.com/property/") ? "noopener noreferrer" : undefined} onClick={() => { if (!item.read) void markRead(item); }}>{notificationActionLabel(item.notificationType)}</Link></Button>}
