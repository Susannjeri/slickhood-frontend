...
import { deliveryLabel, notificationText } from "@/lib/notification-display";
...
export function MyNotifications() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (currentRequest !== requestId.current) return;
      setError(apiErrorMessage(error, "Could not load your notifications."));
    } finally {
  ...
  return <div className="mx-auto max-w-4xl space-y-6 p-4 sm:p-6">
    ...
    {error ? <div role="alert" className="rounded-xl border bg-white p-6"><p>{error}</p><Button className="mt-3" onClick={() => void load()}>Try again</Button></div> : loading && items.length === 0 ? <div className="flex justify-center rounded-xl border bg-white py-20"><Loader2 className="h-8 w-8 animate-spin text-[#EF4217]" /></div> :
      ...
      <div className="space-y-3">{items.map(item => <article key={item.id} className={`rounded-xl border p-5 shadow-sm ${item.read ? "bg-white" : "border-orange-200 bg-orange-50/40"}`}>
        <div className="flex flex-wrap items-start justify-between gap-3"><div className="flex items-center gap-2"><span className="text-slate-500">{item.channel === "EMAIL" ? <Mail className="h-4 w-4"/> : <MessageSquare className="h-4 w-4"/>}</span><strong>{item.notificationType.replaceAll("_", " ")}</strong>{!item.read && <Badge className="bg-[#EF4217]">New</Badge>}</div><Badge variant={item.delivered ? "default" : "outline"}>{item.delivered ? <CheckCircle2 className="mr-1 h-3 w-3"/> : <Clock3 className="mr-1 h-3 w-3"/>}{deliveryLabel(item)}</Badge></div>
