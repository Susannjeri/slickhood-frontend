...
function summaryValue(value:unknown):AffiliateSummary|null{
  if(!value||typeof value!=="object"||Array.isArray(value))return null;
  const source=value as Record<string,unknown>,rawProfile=source.profile;
  if(!rawProfile||typeof rawProfile!=="object"||Array.isArray(rawProfile))return null;
  const profile=rawProfile as Record<string,unknown>,userId=numeric(source.userId,Number.NaN);
  if(!Number.isFinite(userId))return null;
  return {userId,name:text(source.name)||undefined,email:text(source.email)||undefined,referrals:numeric(source.referrals),conversions:numeric(source.conversions),profile:{referralCode:text(profile.referralCode),status:text(profile.status,"UNKNOWN"),commissionRate:numeric(profile.commissionRate),minimumPayout:numeric(profile.minimumPayout),currency:text(profile.currency,"KES"),payoutAccountId:Number.isFinite(Number(profile.payoutAccountId))?Number(profile.payoutAccountId):undefined,appliedAt:text(profile.appliedAt)||undefined,reviewedAt:text(profile.reviewedAt)||undefined,reviewedByUserId:Number.isFinite(Number(profile.reviewedByUserId))?Number(profile.reviewedByUserId):undefined,reviewNotes:text(profile.reviewNotes)||undefined}};
}
function detailValue(value:unknown):AffiliateDetail|null{
  if(!value||typeof value!=="object"||Array.isArray(value))return null;
  const source=value as Record<string,unknown>,affiliate=summaryValue(source.affiliate);
  if(!affiliate)return null;
  const balances=Array.isArray(source.balances)?source.balances.flatMap(item=>{if(!item||typeof item!=="object"||Array.isArray(item))return [];const b=item as Record<string,unknown>;return [{currency:text(b.currency,"KES"),available:numeric(b.available),pending:numeric(b.pending),lifetime:numeric(b.lifetime),pendingPayouts:numeric(b.pendingPayouts),payoutSupported:b.payoutSupported===true}] as AffiliateDetail["balances"];}):[];
...
export default function AffiliateDirectory(){
  ...
  useEffect(()=>{let current=true;setBusy(true);setError(null);void affiliateDirectory({query:search||undefined,status:status||undefined,page}).then(r=>{if(current){setRows(envelopePageList<unknown>(r).map(summaryValue).filter((item):item is AffiliateSummary=>item!==null));setPages(Math.max(0,numeric(r.data?.totalPages)));setTotal(Math.max(0,numeric(r.data?.totalElements)))}}).catch(e=>{if(current)setError(apiErrorMessage(e,"Affiliate directory could not be loaded."))}).finally(()=>{if(current)setBusy(false)});return()=>{current=false}},[search,status,page,retry]);
  async function open(row:AffiliateSummary){const request=++detailRequest.current;setLoadingDetail(true);setDetailError(null);setEditing(false);setDetail({affiliate:row,balances:[]});try{const r=await affiliateDetail(row.userId);const next=detailValue(envelopeItem<unknown|null>(r,null));if(!next)throw new Error("Affiliate detail response was empty or invalid");if(request===detailRequest.current)setDetail(next)}catch(e){if(request===detailRequest.current)setDetailError(apiErrorMessage(e,"Affiliate details could not be loaded. Close and retry."))}finally{if(request===detailRequest.current)setLoadingDetail(false)}}
  ...
  async function save(e:FormEvent){e.preventDefault();if(!detail||saving)return;setSaving(true);setDetailError(null);try{const r=await editAffiliate(detail.affiliate.userId,{status:editStatus,commissionRate:rate,minimumPayout:minimum,reason:reason.trim()});const next=detailValue(envelopeItem<unknown|null>(r,null));if(!next)throw new Error("Affiliate detail response was empty or invalid");setDetail(next);setEditing(false);setRetry(v=>v+1)}catch(e){setDetailError(apiErrorMessage(e,"Affiliate decision could not be saved."))}finally{setSaving(false)}}
  ...
  return <section id="affiliate-directory" className="space-y-4 rounded-xl border bg-white p-5">
    ...
    <Dialog open={!!detail} onOpenChange={v=>{if(!v&&!saving){detailRequest.current++;setDetail(null)}}}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>{detail?.affiliate.profile.status==="PENDING_APPROVAL"?"Review affiliate application":"Manage affiliate"}</DialogTitle><DialogDescription>Only Superadmin can approve access or change financial settings. Previous financial entries and rates remain intact.</DialogDescription></DialogHeader>
      {loadingDetail?<p>Loading affiliate details…</p>:detail&&<>
        ...
        <p className="text-sm text-slate-600">Applied {detail.affiliate.profile.appliedAt&&Number.isFinite(new Date(detail.affiliate.profile.appliedAt).getTime())?new Date(detail.affiliate.profile.appliedAt).toLocaleString():"—"} · Status {detail.affiliate.profile.status.replaceAll("_"," ")}</p>
