...
export default function AffiliateDirectory(){
  ...
  useEffect(()=>{let current=true;setBusy(true);setError(null);void affiliateDirectory({query:search||undefined,status:status||undefined,page}).then(r=>{if(current){setRows(r.data?.data??[]);setPages(r.data?.totalPages??0);setTotal(r.data?.totalElements??0)}}).catch(e=>{if(current)setError(apiErrorMessage(e,"Affiliate directory could not be loaded."))}).finally(()=>{if(current)setBusy(false)});return()=>{current=false}},[search,status,page,retry]);
  async function open(row:AffiliateSummary){const request=++detailRequest.current;setLoadingDetail(true);setDetailError(null);setEditing(false);setDetail({affiliate:row,balances:[]});try{const r=await affiliateDetail(row.userId);if(request===detailRequest.current)setDetail(r.data.data)}catch(e){if(request===detailRequest.current)setDetailError(apiErrorMessage(e,"Affiliate details could not be loaded. Close and retry."))}finally{if(request===detailRequest.current)setLoadingDetail(false)}}
  ...
  return <section id="affiliate-directory" className="space-y-4 rounded-xl border bg-white p-5">
    ...
    {error?<div role="alert">{error}<Button variant="outline" onClick={()=>setRetry(retry+1)}>Retry directory</Button></div>:busy?<p>Loading affiliates…</p>:<>
      ...
      {rows.map(a=><div key={a.userId} className="flex flex-wrap items-center justify-between gap-3 rounded border p-4"><div><b>{a.name??`Affiliate #${a.userId}`}</b><p>{a.email}</p><p>{a.profile.status.replaceAll("_"," ")} · {a.profile.referralCode}</p><p>{a.referrals} referrals · {a.conversions} conversions · commission {a.profile.commissionRate}%</p></div><Button variant="outline" onClick={()=>void open(a)}>{a.profile.status==="PENDING_APPROVAL"?"Review application":"Manage affiliate"}</Button></div>)}
