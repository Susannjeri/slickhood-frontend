...
export default function SmartGateOperations() {
  ...
  return <div className="space-y-5 px-2 py-5">
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <div className="flex gap-2"><select value={propertyId} onChange={e => setPropertyId(Number(e.target.value))} className="rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm">
    ...
    {canManage && <section className="rounded-2xl border bg-white p-5"><h2 className="mb-4 font-semibold text-[#020B2D]">Controllers</h2><div className="grid gap-3 md:grid-cols-2">
      {devices.length ? devices.map(device => <div key={device.deviceCode} className="flex items-center justify-between rounded-xl border p-4"><div className="flex gap-3"><Radio className={device.enabled ? "text-emerald-600" : "text-gray-400"}/><div><p className="font-medium">{device.displayName}</p><p className="text-xs text-gray-500">{device.gateName || "Gate"} · {device.laneName || "Default lane"}</p><p className="mt-1 font-mono text-[10px] text-gray-400">{device.deviceCode}</p></div></div><button onClick={async () => { if(token){await setGateDeviceEnabled(device.deviceCode,!device.enabled,token); await load();}}} className={`rounded-lg px-3 py-1.5 text-xs font-semibold ${device.enabled ? "bg-red-50 text-red-700" : "bg-emerald-50 text-emerald-700"}`}>{device.enabled ? "Disable" : "Enable"}</button></div>) : <p className="text-sm text-gray-500">No gate controllers registered.</p>}
    </div></section>}
    <section className="overflow-hidden rounded-2xl border bg-white"><div className="border-b px-5 py-4"><h2 className="font-semibold text-[#020B2D]">Recent access decisions</h2></div><div className="overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="text-left text-xs uppercase text-gray-500"><th className="px-5 py-3">Time</th><th className="px-5 py-3">Direction</th><th className="px-5 py-3">Decision</th><th className="px-5 py-3">Reason</th><th className="px-5 py-3">Vehicle</th></tr></thead><tbody>
