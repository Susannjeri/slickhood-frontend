...
export default function SmartGateOperations() {
  ...
  const load = useCallback(async () => {
    ...
    setDevices([]); setEvents([]);
    if (!token || !propertyId || !propertyIds.includes(propertyId)) return;
    setError("");
    ...
    if (deviceResult.status === "rejected") setError("Gate controllers could not be loaded. Please retry.");
  ...
  }, [token, propertyId, canManage, propertyIds]);

  useEffect(() => { if (!propertyIds.includes(propertyId)) setPropertyId(propertyIds[0] ?? 0); }, [propertyIds, propertyId]);
  useEffect(() => { void load(); return () => { requestGeneration.current++; }; }, [load]);

  const addDevice = async () => {
    if (!canManage || mutationLock.current || !token || !propertyId || !form.displayName.trim() || !form.ed25519PublicKey.trim()) return;
  ...
  async function toggleDevice(device: GateDevice) {
    if (!token || !canManage || mutationLock.current) return;
    mutationLock.current = true; setBusy(true);
    try { await setGateDeviceEnabled(device.deviceCode, !device.enabled, token); await load(); }
    catch { setError("Controller status could not be changed. Please retry."); }
    finally { mutationLock.current = false; setBusy(false); }
  }
  ...
  return <div className="space-y-5 px-2 py-5">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div><h1 className="text-2xl font-bold text-[#020B2D]">Smart Gate Operations</h1><p className="text-sm text-gray-500">Secure device inventory and an auditable access trail. Configure controllers with a public key only; keep private keys on the device.</p></div>
      <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row"><select aria-label="Gate property" disabled={busy} value={propertyId} onChange={e => setPropertyId(Number(e.target.value))} className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm sm:w-auto">
    ...
    {showAdd && <section className="grid gap-3 rounded-2xl border bg-white p-5 md:grid-cols-2">
      ...
      <button disabled={busy} onClick={addDevice} className="rounded-xl bg-[#020B2D] px-4 py-2 text-sm font-semibold text-white md:col-span-2">Register controller</button>
    </section>}
