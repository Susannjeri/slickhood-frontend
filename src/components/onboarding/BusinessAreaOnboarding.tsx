...
import { ArrowRight, Building2, ChartNoAxesCombined, Check, Loader2, PlusCircle, ShieldCheck, Store, Users } from "lucide-react";
...
export default function BusinessAreaOnboarding({ registrationMode = false }: { registrationMode?: boolean }) {
  ...
  if (!registrationMode && isSuperadmin) return <main className="flex min-h-screen items-center justify-center bg-[#f3f4f6] px-4 py-10 text-[#071744]">
    <section className="w-full max-w-2xl rounded-[28px] border border-orange-200 bg-white p-7 text-center shadow-lg sm:p-10">
      <span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-orange-50 text-[#ff4b1f]"><ShieldCheck className="h-7 w-7" /></span>
      <p className="mt-5 text-xs font-bold uppercase tracking-[0.2em] text-[#ff4b1f]">Protected platform account</p>
      <h1 className="mt-2 text-3xl font-bold">Superadmin access is isolated</h1>
      <p className="mx-auto mt-4 max-w-xl leading-7 text-slate-600">For security and audit separation, a Superadmin account cannot add, receive or assume Landlord or any other customer role. Use a separate customer account for customer operations.</p>
      <button type="button" onClick={() => router.push("/dashboard")} className="mt-7 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#071744] px-6 py-3 font-bold text-white hover:bg-[#ff4b1f]">Return to Superadmin dashboard <ArrowRight className="h-4 w-4" /></button>
    </section>
  </main>;
  ...
  return <Wrapper className={registrationMode ? "w-full text-[#071744]" : "min-h-screen bg-[#f3f4f6] px-4 py-8 text-[#071744] sm:px-6 sm:py-14"}><div className="mx-auto w-full max-w-7xl">
    ...
    <div className="mt-7 grid grid-cols-1 gap-4 sm:mt-9 sm:grid-cols-2 sm:gap-5 xl:grid-cols-3">{businessAreas.map(area => {
      ...
      return <article key={area.id} className="flex min-w-0 flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-orange-300 hover:shadow-lg sm:rounded-[24px] sm:p-5 lg:p-6">
        ...
        <button type="button" disabled={!available || addingArea !== null || Boolean(loadError)} onClick={() => void chooseArea(area)} className="mt-5 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#071744] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#ff4b1f] disabled:cursor-not-allowed disabled:opacity-50 sm:mt-6">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : !registrationMode && !ownedRole ? <PlusCircle className="h-4 w-4" /> : null}{registrationMode ? "Choose this area" : ownedRole ? area.id === "affiliate" ? "Open programme" : "Open & view plans" : area.id === "affiliate" ? "Apply to join" : "Add business area"}{!busy && <ArrowRight className="h-4 w-4 shrink-0" />}</button>
      </article>;
    })}</div>
