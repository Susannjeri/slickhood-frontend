...
export default function BusinessAreaPlans() {
  ...
  if (current) {
    ...
    // The current-subscription endpoint refreshes an elapsed term to EXPIRED or
    // SUSPENDED before returning it, so status is the authoritative render input.
    const live = current.status === "ACTIVE";
    if (!live) return <main className="min-h-screen bg-slate-50 px-5 py-16"><div className="mx-auto max-w-2xl rounded-[32px] border border-amber-200 bg-white p-10 text-center shadow-xl"><ShieldCheck className="mx-auto h-16 w-16 text-amber-500" /><p className="mt-5 text-sm font-bold uppercase tracking-[0.2em] text-amber-600">Subscription {current.status.toLowerCase()}</p><h1 className="mt-2 text-4xl font-bold text-[#071744]">Restore {current.planDetails.displayName}</h1><p className="mt-3 text-slate-500">Your existing data remains safe. Renew or restore this subscription before entering the {area.title} workspace.</p><p className="mt-5 rounded-2xl bg-amber-50 p-4 font-semibold text-amber-700">The previous term ended {endAt ? endAt.toLocaleDateString("en-KE", { dateStyle: "long" }) : "without a recorded end date"}.</p><button onClick={() => router.push("/dashboard/subscriptions")} className="mt-7 inline-flex items-center gap-2 rounded-xl bg-[#ff4b1f] px-7 py-3 font-bold text-white">Review renewal options<ArrowRight className="h-4 w-4" /></button></div></main>;
    return <main className="min-h-screen bg-slate-50 px-5 py-16"><div className="mx-auto max-w-2xl rounded-[32px] border border-emerald-200 bg-white p-10 text-center shadow-xl"><ShieldCheck className="mx-auto h-16 w-16 text-emerald-500" /><p className="mt-5 text-sm font-bold uppercase tracking-[0.2em] text-emerald-600">Subscription active for {propertyPackageArea ? "your property portfolio" : selectedRole.title}</p><h1 className="mt-2 text-4xl font-bold text-[#071744]">{current.planDetails.displayName}</h1><p className="mt-3 text-slate-500">{propertyPackageArea ? "Your shared property subscription covers Rental Management, Estate/Homeowner Management and Property Sales. The unit allowance applies across the complete portfolio." : `This subscription is attached to your ${selectedRole.title} role.`}</p><p className="mt-5 rounded-2xl bg-orange-50 p-4 font-semibold text-[#ff4b1f]">Current term ends {endAt ? endAt.toLocaleDateString("en-KE", { dateStyle: "long" }) : "without expiry"}</p><button onClick={() => router.push(profileLandingHref)} className="mt-7 inline-flex items-center gap-2 rounded-xl bg-[#ff4b1f] px-7 py-3 font-bold text-white">Enter {area.title}<ArrowRight className="h-4 w-4" /></button></div></main>;
  }
    ...
    <main className="min-h-screen bg-[#f4f5f7] px-5 py-10 text-[#071744] sm:py-14">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-7 text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-slate-500">{propertyPackageArea ? `Registration is complete. Start with one ${trialDays}-day free trial covering Rental Management, Estate/Homeowner Management and Property Sales. The unit allowance is shared across your complete property portfolio.` : `Services and Soko use their own merchant package model. It is separate from the property unit tiers and remains attached to your ${selectedRole.title} role.`}</p>
