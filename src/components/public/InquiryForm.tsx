...
export default function InquiryForm({ slug }: { slug: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(`${apiBase()}/public/property-listings/${encodeURIComponent(slug)}/inquiries`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...body, consent: form.get("consent") === "on" }) }).catch(() => null);
  ...
  return <form onSubmit={submit} className="space-y-4">
    <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium">Full name<input name="name" required maxLength={120} autoComplete="name" className="mt-1.5 w-full rounded-xl border border-slate-300 px-3.5 py-3 outline-none focus:border-[#EF4217]" /></label><label className="text-sm font-medium">Phone<input name="phone" maxLength={40} autoComplete="tel" className="mt-1.5 w-full rounded-xl border border-slate-300 px-3.5 py-3 outline-none focus:border-[#EF4217]" /></label></div>
    ...
    {state === "error" && <p role="alert" className="text-sm text-red-600">We could not send your enquiry. Please check your details and try again.</p>}
