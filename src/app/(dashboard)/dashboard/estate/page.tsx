...
function EstateWorkspace() {
  ...
  const hasSelectedUnitLink = Number.isSafeInteger(requestedPropertyId) && requestedPropertyId > 0
    && Number.isSafeInteger(requestedUnitId) && requestedUnitId > 0;
  ...
  const [assignmentUnitId, setAssignmentUnitId] = useState(hasSelectedUnitLink ? String(requestedUnitId) : "");
  ...
  const propertyOptions = usePagedBusinessProperties(canSelectEstate && Boolean(token), undefined, [],
    "SERVICE_CHARGE", hasSelectedUnitLink ? requestedPropertyId : undefined);
  ...
  useEffect(() => {
    if (hasSelectedUnitLink && units.some(unit => unit.unitId === requestedUnitId)) {
