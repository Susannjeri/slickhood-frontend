...
export default function EditPropertyForm() {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (property.mapLocation) {
          const [lat, lng] = property.mapLocation
            .split(",")
        ...
        // Load existing image
        const imagePath = property.imagePathMask + "/" + property.thumbnail; 
        //property.thumbnail contains full path to image, so no need to concatenate with imagePathMask. This is just to maintain the structure in case we need to change it later.
        if (property.thumbnail) {
          setExistingImagePath(property.thumbnail);
          loadExistingImage(property.thumbnail);
        }
      }
    } catch (error: any) {
  ...
  const loadExistingImage = async (imagePath: string) => {
    try {
  ...
  const onSubmit = async (data: PropertyFormData) => {
    ...
    try {
      await editPropertyDetails({
        propertyId: Number(propertyId),
        image: image!, // If no new image, backend should handle keeping old one
        name: data.name,
        type: data.type,
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6 p-6">
      <style jsx global>
        {`
          /* Default focus for text inputs and textareas */
          ...
          /* Prevent react-select inputs from inheriting box-shadow */
          .rs__control {
      ...
      <div className="space-y-1.5">
        <Breadcrumb items={[
          ...
          { label: watch("name") || "Property", href: `/dashboard/property/properties/details/${propertyId}` },
          { label: "Edit" },
        ]} />
        <div className="flex items-center gap-4">
          <Button
            variant="outline"
            onClick={() => router.push(`/dashboard/property/properties/details/${propertyId}`)}
            size="icon"
          >
