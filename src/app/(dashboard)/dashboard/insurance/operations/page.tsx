  ...
  return <div className="min-h-screen bg-slate-50 p-4 sm:p-7">
    <div className="mx-auto max-w-7xl">
      ...
      <Tabs key={initialTab} defaultValue={initialTab} className="mt-6">
        ...
        {canUseApplicationQueue && <TabsContent id="applications-quotes" value="applications" className="scroll-mt-24 space-y-4"><ApplicationQueue items={cases} permissions={{canReview, canQuote, canApprove, canVerify, canIssue}} onAction={setAction} onEvidence={payment=>void openEvidence(payment)} onDispatch={item=>void openDispatch(item)} onHistory={item=>void openCorrespondence(item)} onDocuments={item=>void openCaseDocuments(item)}/></TabsContent>}
...
function ApplicationQueue({items, permissions, onAction,onEvidence,onDispatch,onHistory,onDocuments}: {items: InsuranceCase[]; permissions: {canReview: boolean; canQuote: boolean; canApprove: boolean; canVerify: boolean; canIssue: boolean}; onAction: (action: Action) => void;onEvidence:(payment:InsurancePayment)=>void;onDispatch:(item:InsuranceCase)=>void;onHistory:(item:InsuranceCase)=>void;onDocuments:(item:InsuranceCase)=>void}) {
