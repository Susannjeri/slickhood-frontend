...
function PartnerDialog({open,editing,value,setValue,logo,setLogo,busy,onClose,onSubmit,onDeactivate}:{open:boolean;editing:boolean;value:typeof blankPartner;setValue:(value:typeof blankPartner)=>void;logo:File|null;setLogo:(file:File|null)=>void;busy:boolean;onClose:()=>void;onSubmit:(event:FormEvent)=>void;onDeactivate:()=>void}){return <Dialog open={open} onOpenChange={next=>!next&&onClose()}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><form onSubmit={onSubmit}><DialogHeader><DialogTitle>{editing?"Edit insurance partner":"Add insurance partner"}</DialogTitle><DialogDescription>Add the approved insurer identity and operational email destinations. Deactivation preserves historical quotations and audit records.</DialogDescription></DialogHeader><div className="mt-5 grid gap-4 sm:grid-cols-2">{!editing&&<Field label="Partner code"><Input required pattern="[A-Z][A-Z0-9_]{1,49}" maxLength={50} value={value.code} onChange={event=>setValue({...value,code:event.target.value.toUpperCase().replace(/[^A-Z0-9_]/g,"")})}/></Field>}<Field label="Display name"><Input required maxLength={160} value={value.name} onChange={event=>setValue({...value,name:event.target.value})}/></Field><div className="sm:col-span-2"><Field label="Insurance company logo"><Input type="file" accept="image/jpeg,image/png,image/webp" onChange={event=>setLogo(event.target.files?.[0]??null)}/>{logo&&<span className="text-xs text-muted-foreground">Selected: {logo.name}</span>}{!logo&&value.logoUrl&&<InsuranceBrandLogo src={value.logoUrl} name={value.name||"Insurance company"}/>}<span className="text-xs text-muted-foreground">JPG, PNG or WebP · maximum 2 MB.</span></Field></div><div className="sm:col-span-2"><Field label="Description"><Textarea maxLength={1000} value={value.description} onChange={event=>setValue({...value,description:event.target.value})}/></Field></div><Field label="Quotation email"><Input required type="email" value={value.quotationEmail} onChange={event=>setValue({...value,quotationEmail:event.target.value})}/></Field><Field label="Claims email"><Input type="email" value={value.claimsEmail} onChange={event=>setValue({...value,claimsEmail:event.target.value})}/></Field><Field label="Renewals email"><Input type="email" value={value.renewalsEmail} onChange={event=>setValue({...value,renewalsEmail:event.target.value})}/></Field>{editing&&<label className="flex items-center gap-2 self-end text-sm"><input type="checkbox" checked={value.active} onChange={event=>setValue({...value,active:event.target.checked})}/>Active partner</label>}</div><DialogFooter className="mt-6">{editing&&value.active&&<Button type="button" variant="destructive" disabled={busy} onClick={onDeactivate}>Deactivate</Button>}<Button type="button" variant="outline" onClick={onClose}>Cancel</Button><Button disabled={busy}><Settings2 className="mr-2 size-4"/>{busy?"Saving…":"Save partner"}</Button></DialogFooter></form></DialogContent></Dialog>}
