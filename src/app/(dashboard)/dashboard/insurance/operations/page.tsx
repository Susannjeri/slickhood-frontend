...
type Action =
  ...
  | {kind: "assignment-decision"; item: InsuranceCase; decision: "ACCEPTED" | "DECLINED"}
...
export default function InsuranceOperationsPage() {
  ...
  const [caseDocuments,setCaseDocuments]=useState<{item:InsuranceCase;documents:InsuranceDocument[];loading:boolean;error:string}|null>(null);
  ...
  async function openCaseDocuments(item:InsuranceCase){setCaseDocuments({item,documents:[],loading:true,error:""});try{const documents=await insuranceService.caseDocuments(item.id);setCaseDocuments({item,documents,loading:false,error:""})}catch(error:unknown){setCaseDocuments({item,documents:[],loading:false,error:apiErrorMessage(error,"Attachments could not be loaded.")})}}
  ...
  return <div className="min-h-screen bg-slate-50 p-4 sm:p-7">
    <div className="mx-auto max-w-7xl">
      ...
      <Tabs key={initialTab} defaultValue={initialTab} className="mt-6">
        ...
        {canUseApplicationQueue && <TabsContent id="applications-quotes" value="applications" className="scroll-mt-24 space-y-4"><ApplicationQueue items={cases} currentStaffId={staff.find(person=>person.email.toLowerCase()===signedInEmail?.toLowerCase())?.id} permissions={{canReview, canQuote, canApprove, canVerify, canIssue}} onAction={setAction} onEvidence={payment=>void openEvidence(payment)} onDispatch={item=>void openDispatch(item)} onHistory={item=>void openCorrespondence(item)} onDocuments={item=>void openCaseDocuments(item)}/></TabsContent>}
    ...
    <Dialog open={Boolean(caseDocuments)} onOpenChange={open=>!open&&setCaseDocuments(null)}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl"><DialogHeader><DialogTitle>Proposal attachments · {caseDocuments?.item.reference}</DialogTitle><DialogDescription>Open only the documents required for this application. Links are short-lived and permission-controlled.</DialogDescription></DialogHeader><div className="mt-4 space-y-3">{caseDocuments?.loading?<p className="text-sm text-muted-foreground">Loading secure attachments…</p>:caseDocuments?.error?<p role="alert" className="rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">{caseDocuments.error}</p>:caseDocuments?.documents.length?caseDocuments.documents.map(document=><div key={document.id} className="flex items-center justify-between gap-3 rounded-xl border p-4"><div><strong>{title(document.category)}</strong><p className="text-sm text-muted-foreground">{document.displayName}</p></div><Button asChild size="sm" variant="outline"><a href={document.downloadUrl} target="_blank" rel="noopener noreferrer"><ExternalLink className="mr-2 size-4"/>Open</a></Button></div>):<Empty text="No attachments have been uploaded for this application."/>}</div><DialogFooter><Button variant="outline" onClick={()=>setCaseDocuments(null)}>Close</Button></DialogFooter></DialogContent></Dialog>
...
function ApplicationQueue({items, currentStaffId, permissions, onAction,onEvidence,onDispatch,onHistory,onDocuments}: {items: InsuranceCase[]; currentStaffId?:number; permissions: {canReview: boolean; canQuote: boolean; canApprove: boolean; canVerify: boolean; canIssue: boolean}; onAction: (action: Action) => void;onEvidence:(payment:InsurancePayment)=>void;onDispatch:(item:InsuranceCase)=>void;onHistory:(item:InsuranceCase)=>void;onDocuments:(item:InsuranceCase)=>void}) {
  ...
  return items.map(item => <Card key={item.id}>
    ...
    <CardContent>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        {permissions.canReview && item.status==="ASSIGNMENT_PENDING" && item.assignedAdviserId===currentStaffId && <><Button size="sm" onClick={()=>onAction({kind:"assignment-decision",item,decision:"ACCEPTED"})}>Accept assignment</Button><Button size="sm" variant="outline" onClick={()=>onAction({kind:"assignment-decision",item,decision:"DECLINED"})}>Decline assignment</Button></>}
        ...
        {permissions.canReview&&<Button size="sm" variant="ghost" onClick={()=>onHistory(item)}><Inbox className="mr-2 size-4"/>Correspondence</Button>}{permissions.canReview&&<Button size="sm" variant="ghost" onClick={()=>onDocuments(item)}><FileCheck2 className="mr-2 size-4"/>Attachments</Button>}
...
function ActionDialog(props: {action: Action | null; busy: boolean; staff: InsuranceStaff[]; companies: InsuranceCompany[]; adviserId: string; setAdviserId: (value: string) => void; caseStatus: string; setCaseStatus: (value: string) => void; note: string; setNote: (value: string) => void; reference: string; setReference: (value: string) => void; quote: typeof blankQuote; setQuote: (value: typeof blankQuote) => void; quoteFile:File|null;setQuoteFile:(file:File|null)=>void;policy: {policyNumber: string; startDate: string; endDate: string}; setPolicy: (value: {policyNumber: string; startDate: string; endDate: string}) => void; claimStatus: string; setClaimStatus: (value: string) => void; insurerReference: string; setInsurerReference: (value: string) => void; renewalStatus: string; setRenewalStatus: (value: string) => void; onClose: () => void; onSubmit: (event: FormEvent) => void}) {
  ...
  return <Dialog open onOpenChange={open => !open && props.onClose()}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><form onSubmit={props.onSubmit}><DialogHeader><DialogTitle>{heading}</DialogTitle><DialogDescription>Changes are validated by the insurance lifecycle and recorded in the audit history.</DialogDescription></DialogHeader><div className="mt-5 grid gap-4">
    ...
    {action.kind === "assignment-decision" && <>{action.decision==="ACCEPTED"?<p className="rounded-lg border bg-slate-50 p-4 text-sm">Accept responsibility for <strong>{action.item.reference}</strong>? Your decision is retained in the audit history.</p>:<Field label="Reason for declining"><Textarea required maxLength={500} value={props.note} onChange={event=>props.setNote(event.target.value)} placeholder="Explain why this application should return to the assignment queue."/></Field>}</>}
