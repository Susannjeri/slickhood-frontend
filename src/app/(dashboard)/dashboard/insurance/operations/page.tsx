...
export default function InsuranceOperationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [summaryResult, casesResult, claimsResult, renewalsResult,renewalJourneysResult, companiesResult, staffResult,accountsResult] = requests;
  ...
  async function dispatchQuoteRequest(event:FormEvent){event.preventDefault();if(!dispatchCase||!dispatchCompany)return;setBusy(true);try{await insuranceService.requestInsurerQuote(dispatchCase.id,dispatchCompany);toast.success("Proposal sent to the insurer. Replies will appear in correspondence.");setDispatchCase(null);setDispatchCompany("")}catch(error:unknown){toast.error(apiErrorMessage(error,"The proposal could not be sent. Check that the insurer has a quotation email."))}finally{setBusy(false)}}
  ...
  return <div className="min-h-screen bg-slate-50 p-4 sm:p-7">
    <div className="mx-auto max-w-7xl">
      ...
      <Tabs defaultValue={defaultTab} className="mt-6">
        ...
        {canUseApplicationQueue && <TabsContent value="applications" className="space-y-4"><ApplicationQueue items={cases} permissions={{canReview, canQuote, canApprove, canVerify, canIssue}} onAction={setAction} onEvidence={payment=>void openEvidence(payment)} onDispatch={item=>{setDispatchCase(item);setDispatchCompany(companies.find(company=>company.active)?.code??"")}} onHistory={item=>void openCorrespondence(item)}/></TabsContent>}
    ...
    <PartnerDialog open={partnerOpen} editing={Boolean(editingPartner)} value={partner} setValue={setPartner} busy={busy} onClose={()=>setPartnerOpen(false)} onSubmit={savePartner} onDeactivate={deactivatePartner}/>
  </div>;
}
...
function PartnerDialog({open,editing,value,setValue,busy,onClose,onSubmit,onDeactivate}:{open:boolean;editing:boolean;value:typeof blankPartner;setValue:(value:typeof blankPartner)=>void;busy:boolean;onClose:()=>void;onSubmit:(event:FormEvent)=>void;onDeactivate:()=>void}){return <Dialog open={open} onOpenChange={next=>!next&&onClose()}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><form onSubmit={onSubmit}><DialogHeader><DialogTitle>{editing?"Edit insurance partner":"Add insurance partner"}</DialogTitle><DialogDescription>Only approved brand assets and operational destinations should be published. Deactivation is used instead of permanent deletion so historical quotations remain auditable.</DialogDescription></DialogHeader><div className="mt-5 grid gap-4 sm:grid-cols-2">{!editing&&<Field label="Partner code"><Input required pattern="[A-Z][A-Z0-9_]{1,49}" maxLength={50} value={value.code} onChange={event=>setValue({...value,code:event.target.value.toUpperCase().replace(/[^A-Z0-9_]/g,"")})}/></Field>}<Field label="Display name"><Input required maxLength={160} value={value.name} onChange={event=>setValue({...value,name:event.target.value})}/></Field><div className="sm:col-span-2"><Field label="Logo URL"><Input maxLength={800} placeholder="/insurance/brands/partner.webp or https://…" value={value.logoUrl} onChange={event=>setValue({...value,logoUrl:event.target.value})}/></Field></div><div className="sm:col-span-2"><Field label="Description"><Textarea maxLength={1000} value={value.description} onChange={event=>setValue({...value,description:event.target.value})}/></Field></div><Field label="Quotation email"><Input type="email" value={value.quotationEmail} onChange={event=>setValue({...value,quotationEmail:event.target.value})}/></Field><Field label="Claims email"><Input type="email" value={value.claimsEmail} onChange={event=>setValue({...value,claimsEmail:event.target.value})}/></Field><Field label="Renewals email"><Input type="email" value={value.renewalsEmail} onChange={event=>setValue({...value,renewalsEmail:event.target.value})}/></Field>{editing&&<label className="flex items-center gap-2 self-end text-sm"><input type="checkbox" checked={value.active} onChange={event=>setValue({...value,active:event.target.checked})}/>Active partner</label>}</div><DialogFooter className="mt-6">{editing&&value.active&&<Button type="button" variant="destructive" disabled={busy} onClick={onDeactivate}>Deactivate</Button>}<Button type="button" variant="outline" onClick={onClose}>Cancel</Button><Button disabled={busy}><Settings2 className="mr-2 size-4"/>{busy?"Saving…":"Save partner"}</Button></DialogFooter></form></DialogContent></Dialog>}
