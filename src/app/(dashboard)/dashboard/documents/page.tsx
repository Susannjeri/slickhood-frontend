  ...
  return <div className="mx-auto w-full max-w-7xl space-y-6 p-4 sm:p-6">
    ...
    {canCreate && <Card><CardHeader><CardTitle className="flex items-center gap-2"><FilePlus2 className="h-5 w-5 text-[#EF4217]" />Create draft</CardTitle>
      ...
      <CardContent><form onSubmit={generate} className="grid gap-4 md:grid-cols-3">
        ...
        <div className="space-y-2 md:col-span-3"><Label htmlFor="document-reason">Custom clauses / transaction-specific terms</Label><Textarea id="document-reason" maxLength={1000} value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Optional. The standard approved template is used when left blank." /><p className="text-xs text-muted-foreground">These terms are saved inside this immutable document version. Material legal changes should be reviewed before issue.</p></div>
    ...
    {canEditTemplates && <Card><CardHeader><CardTitle>Standard and reusable templates</CardTitle><CardDescription>“Edit” creates a new version. Existing issued documents never change. Keep legal review enabled until counsel approves the wording.</CardDescription></CardHeader>
