...
function HelpDeskWorkspace() {
  ...
  const importManual = async () => {
    ...
    mutationLock.current = true; setSending(true);
    try {
      const result = unwrap<{ created: number; updated: number }>(await importHelpManualDrafts())[0];
      if (!result) throw new Error("Missing import result");
      toast.success(`${result.created} manual articles added; ${result.updated ?? 0} existing articles synchronised. Publication approvals were preserved.`);
      await load();
    } catch { toast.error("The governed manual could not be synchronised. Existing publication approvals were not changed."); }
    finally { mutationLock.current = false; setSending(false); }
  };
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1500px] flex-col gap-5 p-4 pb-10 md:p-6">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {mode === "knowledge" && canManageArticles && <Button disabled={sending} onClick={importManual}>Synchronise user manual</Button>}
      ...
      {mode === "knowledge" ? (
        <KnowledgeView articles={filteredArticles} search={search} setSearch={setSearch} loading={loading}
          isAdmin={canManageArticles} onNew={() => setArticleEditor(emptyArticle())} onEdit={setArticleEditor} />
      ) : (
        <>
      ...
      {articleEditor && <ArticleDialog article={articleEditor} onClose={() => setArticleEditor(undefined)} onSaved={() => { setArticleEditor(undefined); void load(); }} />}
    </div>
  );
}
...
function KnowledgeView({articles,search,setSearch,loading,isAdmin,onNew,onEdit}:{articles:HelpDeskArticle[];search:string;setSearch:(v:string)=>void;loading:boolean;isAdmin:boolean;onNew:()=>void;onEdit:(a:HelpDeskArticle)=>void}) {
  return <Card><CardHeader className="flex-col gap-3 border-b md:flex-row md:items-center md:justify-between"><div><CardTitle>Help knowledge</CardTitle><p className="mt-1 text-sm text-slate-500">Approved guidance used by customers, agents, and Slickhood Help.</p></div><div className="flex gap-2"><div className="relative"><Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400" /><Input className="pl-9" value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search articles" /></div>{isAdmin&&<Button onClick={onNew}>New article</Button>}</div></CardHeader><CardContent className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">{loading?<Loader/>:articles.map((a)=><article key={a.id} className="rounded-2xl border bg-white p-5"><div className="mb-3 flex items-center justify-between"><Badge variant="secondary">{a.category}</Badge>{isAdmin&&<Badge variant={a.published?"default":"outline"}>{a.published?"Published":"Draft"}</Badge>}</div><p className="mb-1 text-xs text-slate-500">Article {String(a.serialNumber ?? a.id).padStart(2,"0")}{isAdmin&&<span> · Reference {a.id}</span>}</p><h2 className="font-semibold text-[#141130]">{a.title}</h2><p className="mt-2 whitespace-pre-line text-sm leading-6 text-slate-600">{a.body}</p>{isAdmin&&<Button className="mt-4" variant="outline" size="sm" onClick={()=>onEdit(a)}>Edit</Button>}</article>)}</CardContent></Card>;
}
...
function ArticleDialog({article,onClose,onSaved}:{article:HelpDeskArticle;onClose:()=>void;onSaved:()=>void}){
  ...
  const save=async()=>{setSaving(true);try{const{id,...payload}=draft;delete payload.serialNumber;unwrap<HelpDeskArticle>(await saveHelpArticle(payload,id||undefined));toast.success("Help article saved.");onSaved()}catch(error:unknown){toast.error(errorMessage(error,"Article could not be saved."))}finally{setSaving(false)}};
