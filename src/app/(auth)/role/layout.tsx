...
export default function AuthLayout({ children }: { children: ReactNode }) {
  return (
    <div className="min-h-screen w-full overflow-x-clip bg-[#F4F6FB] dark:bg-[#0D0B1F] flex flex-col transition-colors">
      ...
      <main className="flex-1 px-3 py-4 sm:px-6 sm:py-8">
        <div className="relative isolate mx-auto flex w-full max-w-7xl">
          <BackgroundPattern />
          ...
          {/* Card */}
          <div className="min-w-0 flex-1 rounded-2xl border border-gray-100 bg-white px-4 py-5 shadow-sm transition-colors dark:border-white/10 dark:bg-[#1A1740] sm:rounded-l-none sm:px-7 sm:py-7 lg:px-9">
            {children}
          </div>
        </div>
      </main>
      ...
      <footer className="w-full bg-white dark:bg-[#141130] border-t border-gray-100 dark:border-white/10 transition-colors">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex flex-col lg:flex-row items-center justify-between gap-3">
          ...
          <nav className="flex max-w-full flex-wrap items-center justify-center gap-x-4 gap-y-2 text-xs text-gray-500 dark:text-gray-400 sm:gap-x-5">
