...
test("customer can submit a minimal Silverwood quote request",async({context,page})=>{
 ...
 await dialog.getByLabel("Registration number").fill("KDA 123A");await dialog.getByLabel("Vehicle make and model").fill("Toyota Fielder");await dialog.getByLabel("Year of manufacture").fill("2019");await dialog.getByLabel("Primary driver's age").fill("36");await dialog.getByLabel("Estimated value").fill("1000000");
 await dialog.getByRole("button",{name:"Add another"}).click();await dialog.getByLabel("Registration number").nth(1).fill("KDB 456B");await dialog.getByLabel("Vehicle make and model").nth(1).fill("Mercedes Actros");await dialog.getByLabel("Year of manufacture").nth(1).fill("2021");await dialog.getByLabel("Primary driver's age").nth(1).fill("42");await dialog.getByLabel("Estimated value").nth(1).fill("500000");
 ...
 expect(submitted).toMatchObject({productCode:"MOTOR",subjectType:"VEHICLE",sumInsured:1500000,coverStartDate:coverStart,proposalData:{vehicles:[{insuredType:"VEHICLE",registrationNumber:"KDA 123A",makeModel:"Toyota Fielder",manufactureYear:"2019",driverAge:"36",estimatedValue:"1000000",specialType:""},{insuredType:"VEHICLE",registrationNumber:"KDB 456B",makeModel:"Mercedes Actros",manufactureYear:"2021",driverAge:"42",estimatedValue:"500000",specialType:""}]}});
});
...
test("insurance manager maintains partner branding and verified payment routes",async({context,page})=>{
 ...
 await expect(page.getByRole("heading",{name:"Edit insurance company"})).toBeVisible();
