...
test("insurance renewal runs from Silverwood offer through verified payment and renewed policy",async({context,page})=>{
 ...
 await page.getByRole("button",{name:"Verify renewal payment"}).click();await page.getByRole("button",{name:"Confirm"}).click();await page.getByRole("button",{name:"Record renewal remittance"}).click();await page.getByLabel("Insurer remittance reference").fill("APA-REN-REM-51");await page.getByRole("button",{name:"Confirm"}).click();await page.getByRole("button",{name:"Issue renewed policy"}).click();await page.getByLabel("Renewed policy number").fill("APA-POL-31-R");await page.getByLabel("Renewed insurer policy schedule").setInputFiles({name:"renewed-policy.pdf",mimeType:"application/pdf",buffer:Buffer.from("%PDF-1.4 renewed policy")});await page.getByRole("button",{name:"Confirm"}).click();await expect.poll(()=>renewed).toBe(true);
});
